...
function Assert-ArchiveMemberSet {
    ...
    try {
        $expectedNames = @('BoundaryLens.ps1', 'LICENSE', 'README.md', 'SHA256SUMS.txt')
        ...
        foreach ($name in @('BoundaryLens.ps1', 'LICENSE', 'README.md', 'SHA256SUMS.txt')) {
...
function Assert-Sha256Sums {
    ...
    if ($lines.Count -ne 3) {
        Stop-Package 'SHA256SUMS.txt must contain exactly the script, LICENSE and README hashes.'
    }

    $actualHashes = @{}
    foreach ($line in $lines) {
        $match = [regex]::Match($line, '^(?<hash>[A-F0-9]{64})  (?<name>BoundaryLens\.ps1|LICENSE|README\.md)$')
    ...
    foreach ($name in @('BoundaryLens.ps1', 'LICENSE', 'README.md')) {
...
Set-Content -LiteralPath $sumsPath -Value @(
    "$($expectedHashes['BoundaryLens.ps1'])  BoundaryLens.ps1",
    "$($expectedHashes['LICENSE'])  LICENSE",
    "$($expectedHashes['README.md'])  README.md"
) -Encoding utf8 -NoNewline:$false
$expectedHashes['SHA256SUMS.txt'] = Get-Sha256Hex -Path $sumsPath

Compress-Archive -LiteralPath @($standalonePath, $packagedLicensePath, $packagedReadmePath, $sumsPath) -DestinationPath $archivePath -CompressionLevel Optimal -ErrorAction Stop

foreach ($artifactPath in @($standalonePath, $archivePath, $packagedLicensePath, $packagedReadmePath, $sumsPath)) {
