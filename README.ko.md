...
Boundary Lens는 Windows에서 작업 폴더 하나와 접근에 실패한 경로 하나를 지정해 경로, 재분석 지점, ACL 증거를 읽는 진단 도구입니다. Apache-2.0으로 공개한 실험 단계 소프트웨어입니다. 보고서는 권한 허용 여부나 보안을 인증하지 않으며 시스템을 복구하지도 않습니다. 종료 코드 `0`은 보고서를 만들었다는 뜻입니다.
...
확장 기능의 볼륨 조회는 실패 경로에 **선언된 드라이브**만 측정합니다(`observation_scope: declared-path-drive`). 정션 등으로 실제 대상이 다른 볼륨에 있을 수 있으며, 두 볼륨의 관계는 `unknown`으로 남깁니다. JSON·HTML 전달본은 최종 대상 볼륨을 측정했다고 주장하지 않습니다. 필요하면 대상 볼륨 증거를 별도로 확보하세요.
...
현재 `.2` 릴리스 ZIP에는 위 **검증과 압축 해제** 절에 나열한 8개 파일이 들어가고 사건 동반 도구도 포함됩니다. 이전 `.1` ZIP은 4개 파일이며 사건 동반 도구가 없습니다. 내려받은 릴리스에 맞는 체크섬과 파일 목록을 확인하세요. 과거 비공개 릴리스의 파일명·해시·구성을 재사용하지 마세요. 이 저장소는 별도의 공개 이력으로 시작했으며 비공개 개발 기록과 산출물은 공개 릴리스에 포함되지 않습니다.
